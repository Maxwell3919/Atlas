...
def main():
    ...
    source_names=['lambda.in','lambda.dat','lambda.out','alpha2F.dat','al.elph.out','q2r.out','matdyn-dos.out',
                  'al.dense.in','al.scf.in','al.elph.in','q2r.in','matdyn-dos.in']
    ...
    for iq,(q,name,block) in enumerate(zip(qrows,files,qblocks),1):
        ...
        block_freq=[float(v) for v in re.findall(r'freq\s*\(\s*\d+\)\s*=\s*([-\d.]+)\s*\[THz\]',block)]
        assert len(block_freq)==3
        native_ph_frequencies.extend(block_freq)
        ...
        native_q=list(map(float,block.splitlines()[0].split()[:3]))
        assert all(abs(native_q[i]-q[i])<1e-6 for i in range(3))
        assert all(v<=0 or abs(math.sqrt(v)*QE_RY_THZ-f)<1e-4 for v,f in zip(w2,block_freq))
    ...
    native_tc=(r/'lambda.out').read_text().split('lambda        omega_log          T_c')[-1]
    ...
    backups=sorted(r.glob('al.a2Fsave.k*'))
    assert len(backups)==1, 'Expected one dense-grid backup in this branch'
    assert density_hash==hashlib.sha256(backups[0].read_bytes()).hexdigest()
