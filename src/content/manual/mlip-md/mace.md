...
`timestep` 控制每次积分向前走多远，`taut` 控制与热浴交换能量的时间尺度。本例的 100 fs 相当于 100 个积分步；缩短 `taut` 会加强温控耦合，修改 `timestep` 则要通过后面的 NVE 能量对照检查积分误差。初速度种子 `20260922` 与热浴随机种子 `923` 分开保存，因此更换初态和更换热浴随机过程是两种不同的对照。
